...
public class MyScrollView_Focus extends ScrollView {
        ...
        public void handleMessage(Message msg) {
            if(msg.what == 1){
                ...
                if(newY != scrollDistanceY){ //如果不等与手指离开后的Y值，说明滑动还没停止，继续获取新值
                    ...
                    handler.sendEmptyMessageDelayed(1, 5);  //滑动没停止，再次获取
                }
            }
        }
    };

    public MyScrollView_Focus(Context context) {
        super(context);
    }

    public MyScrollView_Focus(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public MyScrollView_Focus(Context context, AttributeSet attrs, int defStyleAttr) {
