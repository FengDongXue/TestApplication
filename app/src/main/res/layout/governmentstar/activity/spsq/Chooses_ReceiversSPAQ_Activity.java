...
public class Chooses_ReceiversSPAQ_Activity extends BaseActivity implements View.OnClickListener{
    ...
    private Call<Bean_Spsq> call;
    private List<Bean_Spsq.DataBean> deptMap;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        call =api.getspsqAcc(bean.getData().getOpId(),bean.getData().getAccountDeptId());

        call.enqueue(new Callback<Bean_Spsq>() {
            @Override
            public void onResponse(Call<Bean_Spsq> call, Response<Bean_Spsq> response) {
                if(response.body().getData() != null){
                    deptMap =response.body().getData();
                    ...
                    for(int i=0;i<deptMap.size();i++){
                        ...
                        for(int j=0;j<deptMap.get(i).getChildList().size();j++){
                            ...
                            childrenBean.setOpId(deptMap.get(i).getChildList().get(j).getOpId());
                            childrenBean.setOpName(deptMap.get(i).getChildList().get(j).getOpName());
                    ...
                    if(getIntent().getStringExtra("type").equals("zhu")){
                        ...
                        for(int i=0;i<list_tong.size();i++){

                            for(int j=0;j<groupList.size();j++){

                                for(int w=0;w<groupList.get(j).getChildList().size();w++){

                                    if(groupList.get(j).getChildList().get(w).getOpId().equals(list_tong.get(i))){
            ...
            @Override
            public void onFailure(Call<Bean_Spsq> call, Throwable t) {

                Toast.makeText(Chooses_ReceiversSPAQ_Activity.this,"网络连接有误",Toast.LENGTH_SHORT).show();
                    ...
                    for(int j=0;j<groupList.get(i).getChildList().size();j++){
                        ...
                        if(groupList.get(i).getChildList().get(j).getChoosed()){
                            ...
                            if(getIntent().getStringExtra("type").equals("zhu")){
                                ...
                                tong = tong +groupList.get(i).getChildList().get(j).getOpId()+",";
                            }
                    }
                }
             }
    ...
    class MyAdapter extends BaseExpandableListAdapter {
        ...
        public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
            ...
            if(getIntent().getStringExtra("type").equals("zhu")){
                ...
                for (int i = 0; i < list_tong.size(); i++) {
                    if (list_tong.get(i).equals(groupList.get(groupPosition).getChildList().get(childPosition).getOpId())) {
                        all_layoutc.setTag(1);
                        isChoose.setVisibility(View.INVISIBLE);
                        name.setTextColor(getResources().getColor(R.color.color_aaa));
                        name.setText(groupList.get(groupPosition).getChildList().get(childPosition).getOpName()+"(不可选择)");
